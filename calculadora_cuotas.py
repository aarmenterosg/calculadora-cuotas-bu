...
monto_str = st.text_input("Monto solicitado (RD$)", value="RD$100,000")

try:
    monto = int(monto_str.replace(",", "").replace("RD$", "").replace("$", "").strip())
except ValueError:
    st.error("Por favor ingresa un monto válido en formato numérico.")
    monto = 0

plazo = st.number_input("Plazo en meses", min_value=1, max_value=120, step=1, value=12)
