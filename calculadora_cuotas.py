...
if 'monto_raw' not in st.session_state:
    st.session_state['monto_raw'] = ""

monto_input = st.text_input("Monto solicitado (RD$)", value=st.session_state['monto_raw'])

try:
    monto = int(monto_input.replace(",", "").replace("RD$", "").replace("$", "").strip())
    st.session_state['monto_raw'] = f"{monto:,}"
except ValueError:
    monto = 0
